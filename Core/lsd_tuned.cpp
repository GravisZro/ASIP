...
static void error(const char * msg)
{
  ...
  //exit(EXIT_FAILURE);
  throw std::logic_error("LSD Error : " + std::string(msg));
}
...
double * LineSegmentDetection( int * n_out,
                               ...
                               int n_bins,
                               int ** reg_img, int * reg_x, int * reg_y, double** used_modulus, double** used_angles)
{
  image_double image;
  ...
  */
  ...
  if (used_modulus != NULL && used_angles != NULL) {
	  *used_modulus = (double*) malloc(sizeof(double) * xsize * ysize);
	  *used_angles = (double*) malloc(sizeof(double) * xsize * ysize);
	  memcpy(*used_modulus, (modgrad->data), sizeof(double) * xsize * ysize);
	  memcpy(*used_angles, (angles->data), sizeof(double) * xsize * ysize);
  }
                           ...
                           int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  double log_eps = -100.0;     /* Detection threshold: -log10(NFA) > log_eps     */
