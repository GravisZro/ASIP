...
void Propagation::propagate_inside_polygon(vector<list<HalfEdge *>>& boundary_edges, Partition* graph, Segment_Regularization_Tree* tree, map<int, double> & angles, vector<int> & bin_to_angle_index, vector<Ray *> & rays, vector<Segment *> & segments, int & n_init_vertices)
{
...
void Propagation::propagate_image_domain(Kinetic_Model *model)
{
	...
	propagate_inside_polygon(boundary_edges, model->graph, model->tree, model->reg_angles, model->bin_to_angle_index, model->rays, model->segments, n_init_vertices);
	graph->reset_indices();
	// free memory
	delete top;
	delete bottom;
