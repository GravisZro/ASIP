...
void Kinetic_Model::init()
{
	...
	if (params->path_input_prediction.empty()) {
		std::cerr << "Error : no semantic input provided. Please provide a semantic map." << std::endl;
		exit(-1);
	}
	
	set_gradient_maps();
	
	set_label_maps(set<pair<int, int>>(), set<pair<int, int>>());
	set_gradient_label_maps();

	std::cout << "The maximal label id is " << params->max_label << "."<< std::endl;
	set_time_string();
    set_basename();
